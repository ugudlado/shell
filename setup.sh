...
configure_git() {
    ...
    # Use environment variables if available, otherwise use existing configuration or prompt
    ...
    if [[ -n "$GIT_USER_NAME" && -n "$GIT_USER_EMAIL" ]]; then
        ...
        log_info "Using git configuration from environment variables"
        log_info "Name: $git_name, Email: $git_email"
    elif [[ -n "$current_name" && -n "$current_email" ]]; then
        log_info "Git already configured with name: $current_name, email: $current_email"
        ...
        # Interactive mode for normal installations
        echo -e "\n${BLUE}=== Git Configuration ===${NC}"

        # Prompt for git name
        if [[ -n "$current_name" ]]; then
            echo -e "${BLUE}Current git name: ${NC}$current_name"
            read -p "Enter your git name (press Enter to keep current): " git_name
            git_name=${git_name:-$current_name}
        else
            read -p "Enter your git name: " git_name
            while [[ -z "$git_name" ]]; do
                read -p "Git name cannot be empty. Please enter your name: " git_name
            done
        fi

        # Prompt for git email
        if [[ -n "$current_email" ]]; then
            echo -e "${BLUE}Current git email: ${NC}$current_email"
            read -p "Enter your git email (press Enter to keep current): " git_email
            git_email=${git_email:-$current_email}
        else
            read -p "Enter your git email: " git_email
            while [[ -z "$git_email" ]]; do
                read -p "Git email cannot be empty. Please enter your email: " git_email
            done
        fi
    fi
    ...
    # Configure VS Code as default git editor and merge tool (skip in container mode)
    if [[ "$DOCKER_MODE" != "true" ]]; then
        ...
        log_info "VS Code set as default editor and merge tool"
    else
        git config --global core.editor "vim"
        log_info "Vim set as default editor for container environment"
    fi
...
apply_dotfiles_with_stow() {
    ...
    echo -e "\n${BLUE}=== Dotfile Management Commands ===${NC}"
    log_info "Edit files: Edit files in $stow_dir/src/home/<package>/"
    log_info "Restow: stow -R -d src -t ~ <package>"
    log_info "Unstow: stow -D -d src -t ~ <package>"
    log_info "Check what's stowed: stow -n -v -d src -t ~ <package>"
...
main() {
    ...
    if [[ "$DOCKER_MODE" != "true" ]]; then
        ...
        if command_exists zsh && [[ "$SHELL" != "$(which zsh)" ]]; then
            ...
            sudo chsh -s "$(which zsh)" "$USER" || log_warning "Could not set zsh as default shell"
        fi
    ...
    echo -e "\n${BLUE}=== Dotfile Management Commands ===${NC}"
    log_info "Edit dotfiles: code $PWD"
    log_info "Sync and restow: cd $PWD && git pull && stow -R -d src -t ~ home"
    log_info "Check status: cd $PWD && git status && stow -n -v -d src -t ~ home"
    log_info "Dry run: cd $PWD && stow -n -v -d src -t ~ home"
    log_info "Unstow: cd $PWD && stow -D -d src -t ~ home"
}
