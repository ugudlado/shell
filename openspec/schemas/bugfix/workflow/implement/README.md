# implement workflow — bugfix

Identical step sequence to `feature-tdd/implement`. Steps execute in `order:` sequence.
Schema-specific override: `implement` (6) requires a regression test before the fix.
...
|     6 | implement         | Implementer writes code for the current task(s) — regression test required before fix |
